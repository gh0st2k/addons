...
$faq_q_title = ""; $faq_a_text = ""; $faq_q_cat_id = "0";
$faq_a_fusion = array();

if (!isset($_GET['q_id']) || !isnum($_GET['q_id'])) { redirect(FAQ_DIR."faq_db.php"); }
if (!isset($_GET['a_id']) || !isnum($_GET['a_id'])) { redirect(FAQ_DIR."faq_db.php"); }

$data = dbarray(dbquery("SELECT faq_q_title FROM ".DB_FAQ_Q." WHERE faq_q_id='".$_GET['q_id']."'"));
opentable("FAQ DB - Eintrag einsenden f&uuml;r: ".$data['faq_q_title']);

$data2 = dbarray(dbquery("SELECT faq_a_text, faq_a_fusion FROM ".DB_FAQ_A."  WHERE faq_a_id='".$_GET['a_id']."'"));
$faq_a_text = $data2['faq_a_text'];
$faq_a_fusion = explode(",", $data2['faq_a_fusion']);

if (isset($_POST['submit'])) {
    $error = "";
    if (isset($_POST['faq_a_fusion']) && is_array($_POST['faq_a_fusion'])) {
		unset($faq_a_fusion);
        $faq_a_fusion = implode(",",stripinput($_POST['faq_a_fusion']));
    ...
    if ($error == "") {
        $result = dbquery("INSERT INTO ".DB_FAQ_A." SET
                            ...
                            faq_a_fusion='".$faq_a_fusion."',
                            faq_a_text='".$faq_a_text."',
                            faq_a_timestamp='".time()."'");
        redirect(FAQ_DIR."faq_center.php?submit_a=ok");
    } else {
...
echo "<form action='".FUSION_SELF."?q_id=".$_GET['q_id']."&amp;a_id=".$_GET['a_id']."' method='post' name='inputform'>\n";
